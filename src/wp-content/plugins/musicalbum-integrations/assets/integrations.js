(function($){
  ...
  // 仪表板数据加载
  if ($('.musicalbum-dashboard-container').length > 0) {
    loadDashboard();
  }
  ...
  function loadDashboard() {
    $.ajax({
      url: ViewingRecords.rest.dashboard,
      method: 'GET',
      headers: { 'X-WP-Nonce': ViewingRecords.rest.nonce }
    }).done(function(data) {
      // 更新统计数据
      $('#stat-total-count').text(data.total_count || 0);
      $('#stat-this-month').text(data.this_month || 0);
      $('#stat-total-spent').text('¥' + (data.total_spent || 0));
      $('#stat-favorite-category').text(data.favorite_category || '暂无');
      
      // 更新最近观演列表
      var recentList = $('#musicalbum-recent-list');
      if (data.recent_viewings && data.recent_viewings.length > 0) {
        var html = '';
        data.recent_viewings.forEach(function(item) {
          ...
          if (item.category) {
            html += '<span class="recent-category">' + escapeHtml(item.category) + '</span>';
          }
          if (item.theater) {
            html += '<span class="recent-theater">' + escapeHtml(item.theater) + '</span>';
          }
          html += '</div>';
          html += '</div>';
        });
        ...
        recentList.html('<div class="musicalbum-dashboard-empty">暂无观演记录</div>');
      }
    }).fail(function() {
      $('#musicalbum-recent-list').html('<div class="musicalbum-dashboard-error">加载失败，请稍后重试</div>');
    });
  }
