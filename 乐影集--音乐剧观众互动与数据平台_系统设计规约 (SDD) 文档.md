...
**功能描述**：在 BuddyPress 用户个人主页集成“观演记录”标签页，展示该用户的观演历史、统计概览及偏好标签。

**业务流程**：
        1. 用户访问个人主页（Profile）
        2. 系统通过 Hook 注入“观演记录”导航项
        3. 点击标签页，调用 `Viewing_Records` 接口获取该用户数据
        4. 渲染用户专属的观演列表与简易统计图
        5. 支持访问者对观演记录进行点赞或评论（通过动态流关联）

+ **个性化资料设置**

**功能描述**：支持用户设置头像、封面图及自定义扩展字段（如“最爱的剧目”、“常驻剧院”）。
